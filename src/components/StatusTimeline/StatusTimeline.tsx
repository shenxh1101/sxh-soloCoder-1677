...
export default function StatusTimeline({
  ...
  const getStatusRecord = (status: ProductionStatus): StatusRecord | undefined => {
    return statusHistory.find((r) => r.status === status);
  };
    ...
    <div className={cn('relative py-4', className)}>
      ...
      <div className="space-y-2">
        {ALL_STATUSES.map((status, index) => {
          ...
          const record = getStatusRecord(status);
          const operator = record ? getUserById(record.operatorId) : undefined;
